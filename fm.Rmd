...
x.id.cross <- read_excel("Case ID Crosswalk-Corrected.xlsx", col_names = FALSE) %>% 
  select(- X__1) %>% 
...
x %>% ggplot(aes(SAT, col.gpa)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~ race, nrow = 2) +
